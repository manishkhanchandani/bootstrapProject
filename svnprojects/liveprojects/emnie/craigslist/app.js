...
.controller('mainController', ['$scope', function($scope) {
	$scope.ref=new Firebase('https://radiant-inferno-4789.firebaseio.com/craigslist');
	$scope.userData = null;
		...
		$scope.ref.authWithOAuthPopup("google", function(error, authData) {
			...
			$scope.ref.child('users').child('uid').once('value', function(snapshot) {
				...
				if (!a) {
					...
					$scope.userData = {'id': id, 'email': email, 'name': displayName, 'img': profileImageURL, 'provider': provider, 'gender': gender, 'link': slink, 'lastName': family_name, 'firstName': given_name, 'createdAt': createdAt, 'lastLoginDt': lastLoginDt};
				    $scope.ref.child('users').child('uid').update($scope.userData);
				
				}
				else {
					var lastLoginDt = Firebase.ServerValue.TIMESTAMP;
					$scope.ref.child('users').child('uid').set(lastLoginDt);
				}
			});
		});
	};
